...
static const UINT8 MxLWare_HYDRA_Version[] = {2, 2, 1, 13, 8};
                              ...
                              UINT8 * matchBufferPtr, UINT8 * maskBufferPtr)
{
  ...
  {
    status = MxLWare_HYDRA_HelperFn_CfgDemodChanTuneParamSetUp(devId, 0, 0, &broadcastWakeCfg.demodChanCfg, chanTuneParamsPtr);

    broadcastWakeCfg.interruptLockFail = interruptLockFail;
...
MXL_STATUS_E MxLWare_HYDRA_API_CfgDevOverwriteDefault(UINT8 devId)
{
  ...
  MXLDBG1(MXL_HYDRA_PRINT("MXL FAIL"););
  }
  if (status == MXL_SUCCESS)
  {
