...
static MXL_STATUS_E MxL_Set_FskMode(UINT8 devId, MXL_HYDRA_FSK_OP_MODE_E fskType)
{
  ...
  BUILD_HYDRA_CMD(MXL_HYDRA_FSK_SET_OP_MODE_CMD, MXL_CMD_WRITE, cmdSize, &fskType, cmdBuff);
  status = MxLWare_HYDRA_SendCommand(devId, cmdSize + MXL_HYDRA_CMD_HEADER_SIZE, &cmdBuff[0]);
...
MXL_STATUS_E MxLWare_HYDRA_API_ReqDiseqcStatus(UINT8 devId, MXL_HYDRA_DISEQC_ID_E diseqcId, UINT32 *statusPtr)
{
  ...
  *statusPtr = 0;
  ...
  if (status == MXL_SUCCESS)
  {
    if (statusPtr)
    {
       //Check for 5x2 SKU which has a different Diseqc mapping for Diseqc ID 1 from the other SKUs
       ...
       {
         MXLDBG2(MXL_HYDRA_PRINT("Hydra SKU is 5x2, disecId1 is modified for this SKU\n"););
         diseqcId = (diseqcId == MXL_HYDRA_DISEQC_ID_1)?MXL_HYDRA_DISEQC_ID_2:MXL_HYDRA_DISEQC_ID_0;
       }
...
MXL_STATUS_E MxLWare_HYDRA_API_CfgDiseqcWrite(UINT8 devId, MXL_HYDRA_DISEQC_TX_MSG_T *diseqcMsgPtr)
{
  ...
  if (mxlStatus == MXL_SUCCESS)
  {
    ...
    {
      MXL_HYDRA_PRINT("diseqcId=%d %d Bytes", diseqcMsgPtr->diseqcId, diseqcMsgPtr->nbyte);
